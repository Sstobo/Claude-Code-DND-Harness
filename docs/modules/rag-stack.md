...
## A campaign can hold more than one book

`/import` (via `agent_extractor.prepare_for_agents`) calls `extract_from_document`
with `clear_existing=True` — the first book replaces whatever was there. To layer a
*second* book into the same campaign (e.g. a world book plus a screenplay that shapes
the PC's voice), use `gm-extract.sh add <file> [campaign]`, which calls
`extract_from_document(clear_existing=False)` and skips the census/chunk-file machinery.
Chunk ids are namespaced per document (`<doc-slug>_NNNN` in `_store_chunks`) so a second
book never overwrites the first's ids; every chunk carries its source in metadata
(`document`). All books share one ChromaDB collection, so `gm-search --rag-only` retrieves
across the whole shelf at once.
