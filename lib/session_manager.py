    ...
    def get_full_context(self, full: bool = False) -> str:
        ...
        if self.get_preferences().get("player_rolls", False):
            lines.append("Dice: PLAYER CHOOSES THE ROLL. When a check is needed, STOP at the "
                         ...
                         "the dice tool with the target and the bonus broken out — "
                         "`uv run python lib/dice.py \"1d20+<total>\" --dc <target> "
                         "--from \"<source>:<N>\"` (one --from per source) — and paste its "
                         "staged block into the narration UNCHANGED: target first, dead air, "
                         "then the result. Never collapse it to one line, (3) narrate "
