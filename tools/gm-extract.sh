...
Commands:
  ...
  add <file> [name]         Additively embed ANOTHER book into a campaign's RAG (multi-book; preserves existing vectors)
...
# Additively embed ANOTHER book into the active campaign's RAG, preserving the
# vectors already there (multi-book worlds). No census, no chunk files, no clear.
add_document() {
    local document="$1"
    local campaign_name
    campaign_name=$(require_campaign "$2" "add") || exit 1

    if [ -z "$document" ]; then
        echo "Error: Document path required" >&2
        echo "  Usage: bash tools/gm-extract.sh add <file> [campaign]" >&2
        exit 1
    fi
    ...
    case "$document" in
        /*) ;;
        *)
            if [ -f "$CALLER_PWD/$document" ]; then document="$CALLER_PWD/$document"
            elif [ -f "$PROJECT_ROOT/$document" ]; then document="$PROJECT_ROOT/$document"
            else echo "Error: File not found: $document" >&2; exit 1; fi
            ;;
    esac
    [ -f "$document" ] || { echo "Error: File not found: $document" >&2; exit 1; }

    if ! $PYTHON_CMD -c "import sentence_transformers; import chromadb" 2>/dev/null; then
        echo "Error: RAG dependencies not installed (uv pip install sentence-transformers chromadb)" >&2
        exit 1
    fi

    local cdir
    cdir=$(campaign_dir "$campaign_name") || exit 1

    echo "Adding book to '$campaign_name' RAG (additive): $document"
    $PYTHON_CMD -c "
import sys; sys.path.insert(0, '$PROJECT_ROOT')
from lib.rag.rag_extractor import RAGExtractor
ex = RAGExtractor('$cdir')
m = ex.extract_from_document('$document', clear_existing=False)
print('Added', m.get('total_chunks'), 'chunks from', m.get('document_name'))
print('Campaign now holds', ex.vector_store.count(), 'vectors total.')
"
}
